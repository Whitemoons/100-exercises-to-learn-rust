...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(t) => t,
        Err(TicketNewError::EmptyString) => panic!("Title cannot be empty"),
        Err(TicketNewError::LongString) => panic!("Title cannot be longer than 50 bytes"),
        Err(TicketNewError::DescriptionError) => Ticket::new(title, "Description not provided".to_string(), status).unwrap()
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::EmptyString);
        }
        if title.len() > 50 {
            return Err(TicketNewError::LongString);
        }
